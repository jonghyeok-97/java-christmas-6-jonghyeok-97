...
public enum December {
    NORMAL(List.of(
            ...
            "크리스마스 디데이 할인:"),
    WEEKDAY(List.of(3, 4, 5, 6, 7, 10, 11, 12, 13, 14, 17, 18, 19, 20, 21, 24, 25, 26, 27, 28, 31), "평일 할인:"),
    ...
    public static String getDecemberMessage(December december) {
        return december.message;
    }

    public static String Message(December december) {
        return Stream.of(December.values())
                .filter(week -> week.equals(december))
                .map(December::getDecemberMessage)
                .findAny()
                .orElse(NONE.message);
    }
}
